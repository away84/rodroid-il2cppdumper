...
pub fn dump_generics(
    ...
    writeln!(file, "// Generated by Il2CppDumper Rust v7.0)?;
    writeln!(file, "// ============================================================================\n")?;
...
fn dump_rgctx_section(
    ...
    let image_results: Vec<(String, String, u64)> = image_defs
        .par_iter()
        .map(|image| {
            ...
            for type_idx in image.type_start as usize..type_end {
                ...
                if let Some(rgctxs) =
                    local_exec.get_rgctx_definition_for_type(&image_name, &type_def, metadata, il2cpp)
                {
                    if !rgctxs.is_empty() {
                        let type_name = local_exec.get_type_def_name(
                            &type_def, type_idx, metadata, il2cpp, true, true,
                        );
                        ...
                        for (i, rgctx) in rgctxs.iter().enumerate() {
                            let formatted = format_rgctx(rgctx, metadata, il2cpp, &mut local_exec);
                            image_buffer.push_str(&format!(
                                "    [{:3}] {} -> {}\n",
                                i,
                ...
                for method_idx in type_def.method_start as usize..method_end {
                    ...
                    if let Some(rgctxs) = local_exec.get_rgctx_definition_for_method(
                        &image_name, &method_def, metadata, il2cpp,
                    ) {
                        if !rgctxs.is_empty() {
                            let type_name = local_exec.get_type_def_name(
                                &type_def, type_idx, metadata, il2cpp, true, true,
                            ...
                            for (i, rgctx) in rgctxs.iter().enumerate() {
                                let formatted =
                                    format_rgctx(rgctx, metadata, il2cpp, &mut local_exec);
                                image_buffer.push_str(&format!(
                                    "    [{:3}] {} -> {}\n",
                                    i,
                                    get_rgctx_type_name(rgctx.rgctx_type),
            ...
            (image_name, image_buffer, local_count)
        })
        .collect();

    let mut total_rgctx_entries = 0u64;
    for (image_name, image_buffer, count) in image_results {
        total_rgctx_entries += count;
...
fn dump_method_specs_section(
    ...
    let lines: Vec<(bool, String)> = (0..total)
        .into_par_iter()
        .map(|i| {
            let mut local_exec = Il2CppExecutor::new_for_worker(executor);
            let (type_name, method_name) =
                local_exec.get_method_spec_name(i, metadata, il2cpp, true);
            let ptr = il2cpp
            ...
            if ptr != 0 {
                ...
                (
                    true,
                    format!("  0x{:08X} | {}::{}", offset, type_name, method_name),
                )
            } else {
                (
                    false,
        ...
        .map(|(img_idx, image)| {
            ...
            for type_idx in image.type_start as usize..type_end {
                ...
                let type_attrs = collect_attributes(
                    metadata, img_idx, type_def.custom_attribute_index, type_def.token,
                );
                ...
                for field_offset in 0..type_def.field_count as usize {
                    ...
                    let field_attrs = collect_attributes(
                        metadata, img_idx, field_def.custom_attribute_index, field_def.token,
                    );
                ...
                for method_idx in type_def.method_start as usize..method_end {
                    ...
                    let method_attrs = collect_attributes(
                        metadata, img_idx, method_def.custom_attribute_index, method_def.token,
                    );
                ...
                for prop_offset in 0..type_def.property_count as usize {
                    ...
                    let prop_attrs = collect_attributes(
                        metadata, img_idx, prop_def.custom_attribute_index, prop_def.token,
                    );
...
fn dump_string_literals_section(
    ...
    let lines: Vec<Option<String>> = (0..total)
        .into_par_iter()
        .map(|i| {
            let value = metadata.get_string_literal_from_index(i).ok()?;
            if value.is_empty() {
                return None;
            }
            let escaped = value
...
fn dump_vtable_section(
    ...
    let image_defs = metadata.image_defs.clone();
    let image_results: Vec<(String, String, u64, u64)> = image_defs
        .par_iter()
        .map(|image| {
            let mut local_exec = Il2CppExecutor::new_for_worker(executor);
            let image_name = metadata.get_string_from_index(image.name_index).unwrap_or_default();
            let mut image_buffer = String::new();
            let mut local_types = 0u64;
            ...
            for type_idx in image.type_start as usize..type_end {
                    ...
                    && (type_def.parent_index as usize) < il2cpp.types.len()
                {
                    ...
                    local_exec.get_type_name(&parent_type, metadata, il2cpp, true, false)
                } else {
                    String::new()
                };
                ...
                for i in 0..type_def.vtable_count as usize {
                    ...
                    if usage == 6 {
                        if (index as usize) < il2cpp.method_specs.len() {
                            ...
                            let (spec_type_name, spec_method_name) = local_exec
                                .get_method_spec_name(index as usize, metadata, il2cpp, true);
                    ...
                    } else if let Some(md) = metadata.method_defs.get(index as usize).cloned() {
                        if md.slot != 0xFFFF {
                            ...
                            {
                                local_exec.get_type_def_name(
                                    &td, declaring_type_idx, metadata, il2cpp, true, true,
                                )
                            } else {
                                "?".to_string()
                            };
            ...
            (image_name, image_buffer, local_types, local_slots)
        })
        .collect();

    let mut total_types_with_vtable = 0u64;
    let mut total_vtable_slots = 0u64;
    for (image_name, image_buffer, types, slots) in image_results {
        total_types_with_vtable += types;
        total_vtable_slots += slots;
        if !image_buffer.is_empty() {
            writeln!(file, "─── Image: {} ───", image_name)?;
            write!(file, "{}", image_buffer)?;
        }
...
fn dump_interface_section(
    ...
    let image_results: Vec<(String, String, u64, u64)> = image_defs
        ...
        .map(|image| {
            ...
            for type_idx in image.type_start as usize..type_end {
                ...
                if type_def.interfaces_count > 0 {
                    ...
                    for i in 0..type_def.interfaces_count as usize {
                            ...
                            && (interface_type_index as usize) < il2cpp.types.len()
                        {
                            ...
                            let iface_name = local_exec.get_type_name(
                                &iface_type, metadata, il2cpp, true, false,
                            );
                            type_buffer.push_str(&format!("      - {}\n", iface_name));
                            local_impls += 1;
                            has_data = true;
                ...
                if type_def.interface_offsets_count > 0 {
                    ...
                    for i in 0..type_def.interface_offsets_count as usize {
                        ...
                        {
                            let iface_type =
                                il2cpp.types[iface_offset.type_index as usize].clone();
                            let iface_name = local_exec.get_type_name(
                                &iface_type, metadata, il2cpp, true, false,
                            );
                            type_buffer.push_str(&format!(
            ...
            (image_name, image_buffer, local_types, local_impls)
        })
        .collect();

    let mut total_types_with_interfaces = 0u64;
    let mut total_interface_impls = 0u64;
    for (image_name, image_buffer, types, impls) in image_results {
        total_types_with_interfaces += types;
        total_interface_impls += impls;
        if !image_buffer.is_empty() {
            writeln!(file, "─── Image: {} ───", image_name)?;
            write!(file, "{}", image_buffer)?;
        }
